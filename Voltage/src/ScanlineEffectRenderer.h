 ...
 public:
  ...
  void add(const Array<Object*>& objects, Camera& camera) {
    ...
    for (uint32_t i = 0; i < objects.getCapacity(); i++) {
      add(objects[i], worldToViewMatrix);
    }
  }

 private:
  void add(Object* object, const Matrix& worldToViewMatrix) {
    Mesh* mesh = object->mesh;

    // Transform all vertices to world coordinate space
    mesh->transformVertices(object->getModelMatrix());

    // Create vertices for all lines clipping XZ plane
    vertices.clear();
    for (uint32_t i = 0; i < mesh->edgeCount; i++) {
      Vector4& a = mesh->edges[i].vertices.a->transformed;
      Vector4& b = mesh->edges[i].vertices.b->transformed;
    ...
    qsort(vertices.getElements(), vertices.getSize(), sizeof(ScanlineRendererVertex), compareFunc);

    // Transform vertices to view space and perspective divide them
    for (uint32_t i = 0; i < vertices.getSize(); i++) {
      ScanlineRendererVertex& vertex = vertices[i];
      Vector4 transformed = Vector4Transform({vertex.world.x, vertex.world.y, vertex.world.z, 1.0},
                                             worldToViewMatrix);
